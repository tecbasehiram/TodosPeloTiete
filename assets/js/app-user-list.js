...
document.addEventListener('DOMContentLoaded', function (e) {
  ...
  const phoneMaskList = document.querySelectorAll('.phone-mask'),
    cpfMask = document.querySelectorAll('.cpf-mask'),
    dataNascMask = document.querySelectorAll('.data-nasc-mask'),
  ...
  // CPF
  if (cpfMask) {
    cpfMask.forEach(cpfField => {
      cpfField.addEventListener('input', event => {
        const cleanValue = event.target.value.replace(/\D/g, '');
        cpfField.value = formatGeneral(cleanValue, {
          blocks: [3, 3, 3, 2],
          delimiters: ['.' , '.', '-']
        });
      });
  ...
  // Data de nascimento
  if (dataNascMask) {
    dataNascMask.forEach(dataNascField => {   
      dataNascField.addEventListener('input', event => {
        const cleanValue = event.target.value.replace(/\D/g, '');
        dataNascField.value = formatGeneral(cleanValue, {
          blocks: [2, 2, 4],
          delimiters: ['/' , '/']
        });
      });
